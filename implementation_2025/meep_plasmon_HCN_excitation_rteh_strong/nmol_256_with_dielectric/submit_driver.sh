...
#SBATCH -J driver_rteh      # Job name
#SBATCH -o driver_rteh.o%j      # Name of stdout output file
#SBATCH -e driver_rteh.e%j      # Name of stderr error file
...
if [ "$nmol" -gt 1 ]; then
    ...
    for ((i=1; i<nmol; i++)); do
        ...
        mxl_driver --model rtehrenfest --address $HOST --port $PORT --param  "molecule_xyz=../HCN_benchmark/hcn.xyz, functional=b3lyp, dft_grid_name=SG1, basis=cc-pvdz, dt_rttddft_au=0.04788, memory=2GB, num_threads=1, n_fock_per_nuc=6, n_elec_per_fock=6" &
    done
    ...
    mxl_driver --model rtehrenfest --address $HOST --port $PORT --param  "molecule_xyz=../HCN_benchmark/hcn.xyz, functional=b3lyp, dft_grid_name=SG1, basis=cc-pvdz, dt_rttddft_au=0.04788, memory=2GB, num_threads=1, n_fock_per_nuc=6, n_elec_per_fock=6" --verbose
fi
