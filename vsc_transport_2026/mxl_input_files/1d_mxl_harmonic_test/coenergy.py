...
def collect_molecular_ids(folder):
    """Map each coenergy_box_N.xyz file index to its assigned 0-based grid ID."""
    box_to_grid = {}
    grid_to_box = {}
    unresolved_boxes = []
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Collect coenergy_box_1.xyz ... coenergy_box_144.xyz into a "
            "10001 x 144 NumPy array, using out_*.lmp MaxwellLink molecular "
            "IDs to place each file on the correct grid point."
        )
    )
    parser.add_argument(
        "folder",
